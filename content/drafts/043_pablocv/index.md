...
- Built and deployed an internal AI Q&A documentation assistant using Amazon Bedrock and AWS Knowledge Base, enabling engineers to query platform documentation through natural language. Implemented a RAG pipeline with document chunking and Elasticsearch-backed retrieval, applied prompt engineering techniques including chain-of-thought and few-shot prompting, and integrated Bedrock Guardrails for content safety. Used pydantic AI for structured model responses and tool definitions. Deployed as a serverless API on AWS Lambda with Amazon API Gateway, with documentation hosted on S3. Evaluated output quality using an LLM-as-judge framework against a curated Q&A dataset sourced from Slack support threads. Logged traces and API activity to Datadog for observability and alerting; stored conversations in S3 for analysis with Athena. Adopted across multiple platform consumer teams and iteratively improved based on user feedback.
